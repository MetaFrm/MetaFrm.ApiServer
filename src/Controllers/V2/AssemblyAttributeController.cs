...
namespace MetaFrm.ApiServer.Controllers.V2
{
    ...
    public class AssemblyAttributeController(ILogger<AssemblyAttributeController> logger, Factory _) : ControllerBase, ICore
    {
        ...
        /// 키와 값의 컬렉션을 나타냅니다.
        /// </summary>
        private static ConcurrentDictionary<string, AssemblyAttributeShort1> AssemblyAttributes { get; set; } = [];
        ...
        public IActionResult? Get(string fullNamespace)
        {
            ...
            if (AssemblyAttributes.TryGetValue(key, out AssemblyAttributeShort1? value))
                return Ok(value);
            ...
            try
            {
                ...
                {
                    AssemblyAttributeShort1? assemblyAttribute;
                    assemblyAttribute = httpResponseMessage.Content.ReadFromJsonAsync<AssemblyAttributeShort1>().Result;
            ...
            if (AssemblyAttributes.TryGetValue(key, out AssemblyAttributeShort1? value2))
                return Ok(value2);
            else
            {
